...
def getdrugurl(herb):
    ...
    '''
    selector = Select(driver.find_element_by_id('searchItemTcm'))

    selector.select_by_index("1")  # 通过index进行选择,index从0开始
    '''
    
    
    selector = Select(driver.find_element_by_id('searchItemTcm').click())
    
    selector.select_by_value("Herb name")
    
    driver.find_element_by_id('searchItemTcm').click()
    
    
    driver.find_element_by_class_name('btn-group bootstrap-select open')
    
    driver.find_element_by_class_name("btn-group bootstrap-select")
